...
describe('lastfm.ts', () => {
  ...
  describe('client errors', () => {
    it('throws validation error if api credentials are missing', () => {
      expect(() => new LastFmClient({ apiKey: '', apiSecret: 'sec' })).toThrow(
        LastFmValidationError
      );
    });
  ...
  describe('pagination generator', () => {
    it('iterates through pages using async generator', async () => {
      ...
      spyOn(client.users, 'getRecentTracks').mockImplementation(
        async (user: string, limit = 50, page = 1) => {
          mockApiCallCount++;
          if (page === 1) {
            return {
              recenttracks: {
